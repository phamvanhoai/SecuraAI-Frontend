...
async function safeJson(response: Response): Promise<unknown> {
  ...
  try {
    ...
    throw new ApiError("Phản hồi máy chủ không hợp lệ.", response.status, "UNKNOWN_ERROR");
  }
}
...
export async function apiRequest<T>(path: string, options: ApiRequestOptions = {}): Promise<T> {
  ...
  if (typeof payload !== "object" || payload === null || !("success" in payload) || !("data" in payload)) {
    throw new ApiError("Phản hồi máy chủ không đúng contract.", response.status, "UNKNOWN_ERROR", payload);
  }

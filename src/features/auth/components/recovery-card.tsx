...
export function RecoveryCard({ step }: { step: Step }) {
  ...
  const [otp, setOtp] = useState(
    tokenFromUrl.padEnd(6, " ").slice(0, 6).split(""),
  );
  ...
  const content = {
    ...
    password: [
      "Reset password",
      "Create a new password to finish the process.",
    ],
    success: [
    ...
    <section className="w-full max-w-[46rem]">
      ...
      <div className="border-border bg-surface rounded-[12px] border p-7 shadow-[0_20px_55px_rgba(28,55,100,0.08)] sm:p-10">
        <div className="text-center">
          <span className="bg-brand-soft text-brand mx-auto grid size-16 place-items-center rounded-full">
        ...
        {message ? (
          <Alert className="border-danger/25 bg-danger-soft text-danger mt-6">
            {message}
          </Alert>
        ) : null}
        {step === "email" ? (
          <EmailForm pending={pending} onSubmit={submitEmail} />
        ) : null}
        {step === "otp" ? (
          <OtpForm otp={otp} setOtp={setOtp} onSubmit={submitOtp} />
        ) : null}
        {step === "password" ? (
          <PasswordForm
            token={tokenFromUrl}
...
function OtpForm({
  ...
  function updateDigit(index: number, value: string): void {
    ...
    const next = otp.map((item, itemIndex) =>
...
function PasswordForm({
  ...
  token: string;
  pending: boolean;
  onSubmit: (values: ConfirmPasswordResetInput) => Promise<void>;
}) {

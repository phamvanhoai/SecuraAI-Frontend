...
function residualAssessmentIssues(
  risk: RiskDetail,
  plan: RiskDetail["treatmentPlans"][number] | undefined,
): string[] {
  if (!plan) return ["An active treatment plan is required."];
  const issues: string[] = [];
  if (!["approved", "in_treatment"].includes(risk.assessment.status))
    issues.push("The risk must be approved or in treatment.");
  if (!["approved", "in_progress", "completed"].includes(plan.status))
    issues.push("The treatment plan is not ready for residual assessment.");
  const activeActions = plan.actions.filter(
    ({ status }) => status !== "cancelled",
  );
  if (
    (plan.strategy !== "accept" && activeActions.length === 0) ||
    activeActions.some(
      ({ status, progressPercent }) =>
        status !== "completed" || progressPercent !== 100,
    )
  )
    issues.push("Complete every active treatment action first.");
  return issues;
}
...
export function RiskAssessmentDetailDialog({
  ...
  const [returningPlan, setReturningPlan] = useState<
    RiskDetail["treatmentPlans"][number] | null
  >(null);
  const [assessingResidualRisk, setAssessingResidualRisk] =
  ...
  const residualPlan = data?.treatmentPlans.find(
    ({ status }) => status !== "cancelled",
  );
  const residualIssues = data
    ? residualAssessmentIssues(data, residualPlan)
    : [];
  const canAssessResidual =
    (session.data?.permissions.includes("risk-assessments.assess-residual") ??
      false) &&
    (isAdmin ||
      data?.assessment.assessedBy?.id === session.data?.id ||
      residualPlan?.owner?.id === session.data?.id);
  return (
    <>
      <Dialog
      ...
      >
        ...
        {detail.isError ? (
          <Alert>
            ...
            <span>
              The assessment may not exist or you may not have access.
            </span>
          </Alert>
        ) : null}
        {data ? (
                  ...
                  {canAssessResidual ? (
                    <div className="mt-4">
                      <Button
                        type="button"
                        variant="secondary"
                        disabled={residualIssues.length > 0}
                        title={residualIssues[0]}
                        onClick={() => setAssessingResidualRisk(data)}
                      >
                        {data.residualRisk
                      ...
                      >
                        ...
                        {plan.approval ? (
                          <div className="bg-info-soft text-info mt-3 rounded-lg p-3 text-xs">
                            <p className="font-medium">
                              Approval:{" "}
                              {plan.approval.status.replaceAll("_", " ")}
                            </p>
                            <p className="mt-1">
                            ...
                            <p className="mt-1">
                              Submitted by{" "}
                              {plan.approval.submittedBy?.fullName ?? "Unknown"}{" "}
                              · {formatDate(plan.approval.submittedAt)}
                            </p>
                            {plan.approval.submissionNote ? (
                              <p className="mt-1">
                                Note: {plan.approval.submissionNote}
                              </p>
                            ) : null}
                            {plan.approval.latestDecision?.decision ===
                            "returned" ? (
                              <div className="border-danger/25 bg-danger-soft text-danger mt-3 rounded-lg border p-3">
                                ...
                                <p className="mt-1 whitespace-pre-wrap">
                                  {plan.approval.latestDecision.comment ??
                                    "No revision reason provided."}
                                </p>
                                <p className="mt-1">
                                  By{" "}
                                  {plan.approval.latestDecision.actedBy
                                    ?.fullName ?? "Unknown"}{" "}
                                  ·{" "}
                                  {formatDate(
                                    plan.approval.latestDecision.actedAt,
                                  )}
                                </p>
                              </div>
                            ) : null}
                              ...
                              <div className="mt-3 border-t border-current/15 pt-3">
                                ...
                                <ol className="mt-2 space-y-2">
                                  {plan.approval.history.map((entry, index) => (
                                    <li
                                      className="bg-background/60 rounded-md p-2"
                                      key={`${entry.approvalRequestId}-${entry.actedAt}-${index}`}
                                    >
                                      ...
                                      <p className="mt-1">
                                        {entry.actedBy?.fullName ?? "Unknown"} ·{" "}
                                        {formatDate(entry.actedAt)}
                                      </p>
                                      {entry.comment ? (
                                        <p className="mt-1 whitespace-pre-wrap">
                                          {entry.comment}
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <p className="text-muted mt-1 text-xs">
                                    Due: {formatDate(action.dueDate)} · Assigned
                                    to:{" "}
      ...
      />
      <PerformResidualRiskAssessmentDialog
        risk={assessingResidualRisk}
        onClose={() => setAssessingResidualRisk(null)}
        onReload={() => {
          setAssessingResidualRisk(null);
          void detail.refetch();
        }}
      />
    </>

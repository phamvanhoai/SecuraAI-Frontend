...
export async function POST(
  ...
  const parsedBody = classifyAssetCriticalitySchema.safeParse(body);
  if (!parsedBody.success) return validationError("Invalid classification data");
  ...
  const base = env.NEXT_PUBLIC_API_BASE_URL.endsWith("/")
    ...
    : `${env.NEXT_PUBLIC_API_BASE_URL}/`;
  const url = new URL(
    `assets/${parsedParams.data.assetId}/classify-criticality`,
    base,
  );

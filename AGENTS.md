...
- `develop` and `main` are permanent long-lived branches. Never delete either branch locally or remotely, including after a successful Pull Request merge.
- Never use `gh pr merge --delete-branch`, GitHub's “Delete branch” action, or any equivalent cleanup when the Pull Request head is `develop` or `main`. In particular, merging `develop` into `main` must leave `develop` intact.
- Before merging, inspect the Pull Request base/head pair. After merging `develop` into `main`, verify with `git ls-remote --heads origin main develop` that both remote branches still exist; repair safely through a Pull Request if they diverge, and never force-push either branch.
